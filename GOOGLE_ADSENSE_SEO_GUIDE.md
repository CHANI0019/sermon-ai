...
| **애드센스 "광고 설정 미리보기" 무한 로딩** | 1) 애드센스 사이트 승인 심사 대기 중(준비 중)이거나<br>2) 브라우저 광고 차단 확장 프로그램(AdBlock 등)이 미리보기 iframe을 차단 | 1) 애드센스 `사이트` 메뉴에서 사이트 검토 완료 대기<br>2) 브라우저의 광고 차단(AdBlock/uBlock) 일시 비활성화<br>3) 미리보기가 안 보여도 하단 **[사이트에 적용]** 버튼을 눌러놓으면 승인 후 자동으로 광고 게재됨 |
...
## 5. 💳 Google AdSense 수익 정산 계좌 & 세금 서류 등록 가이드

구글 애드센스를 통해 발생한 광고 수익을 한국 통장으로 지급받기 위한 **필수 서류, 계좌 등록 방법 및 세금 정보 입력 절차**입니다.

### 1) 정산 기준 및 기본 조건
* **최소 지급 기준액**: 누적 광고 수익 **$100 (USD)** 이상 도달 시 정산.
* **주소 인증 (PIN 번호)**: 누적 수익 **$10** 도달 시 구글에서 입력한 주소지로 6자리 PIN 번호가 담긴 실물 우편이 발송됩니다. (우편 수령 후 애드센스에 PIN 입력 필요)

---

### 2) 수익 수령용 통장 (외화 계좌) 및 필수 입력 정보
애드센스 수익은 달러(USD) 해외 전신송금(Wire Transfer) 방식으로 지급되므로 **달러 수취가 가능한 통장**이 필요합니다.

#### 📌 추천 은행 및 SWIFT 코드
* **카카오뱅크**: 해외 송금 수수료가 저렴하여 애드센스 사용자들이 많이 이용 ($5,000 이하 수수료 약 5,000원)
* **주요 은행 SWIFT-BIC 코드 표**:
...
#### 📝 애드센스 지급 수단 등록 방법 (`결제 > 지급 정보 > 결제 수단 추가`)
1. **예금주의 이름**: 통장/신분증과 동일한 영문 이름 (예: `HONG GILDONG`)
2. **은행 이름**: 영문 은행명 (예: `Kakao Bank`)
3. **SWIFT-BIC**: 해당 은행의 8자리 영문 코드 (예: `KAKOKR22XXX`)
4. **계좌번호**: 하이픈(-) 없이 숫자만 입력

---

### 3) 미국 세금 정보 입력 (W-8BEN 서식)
구글(미국 기업)로부터 미국 외 거주자가 수익을 지급받기 위해 **미국 세금 정보(W-8BEN 양식)** 작성이 필수입니다. (`결제 > 지급 정보 > 세금 정보 관리`)

1. **계좌 유형**: `개인 (Individual)` 선택
2. **미국 시민권자 여부**: `아니요 (No)` 선택
3. **세금 양식 유형**: `W-8BEN` (미국 외 개인/사업자 양식) 선택
4. **납세자 번호 (TIN)**:
   * 개인인 경우: 생략 가능 (또는 주민등록번호 입력)
   * 사업자인 경우: 사업자등록번호 입력
5. **조세 조약 (Tax Treaty)**: 한국-미국 조세조약 혜택 신청 (`예` 선택)
6. **서명**: 영문 실명 입력 후 전자 서명 완료
